...
af_xdp_remove_program (af_xdp_device_t *ad)
{
  ...
  ret = af_xdp_enter_netns (ad->netns, ns_fds);
  if (ret)
    {
      af_xdp_log (VLIB_LOG_LEVEL_ERR, ad, "enter netns failed");
      return ret;
    }
...
static void
af_xdp_rxq_reclaim_fill_queue (vlib_main_t *vm, af_xdp_rxq_t *rxq)
{
  u32 n_submitted = *rxq->fq.producer - *rxq->fq.consumer;
  u32 bis[VLIB_FRAME_SIZE];
  u32 n_free = 0;
  u32 i;

  n_submitted = clib_min (n_submitted, rxq->fq.size);
  for (i = 0; i < n_submitted; i++)
    {
      u32 idx = *rxq->fq.consumer + i;
      u64 addr = *xsk_ring_prod__fill_addr (&rxq->fq, idx);
      af_xdp_buffer_index_from_xdp_addr (bis, &n_free, addr);
      if (n_free == ARRAY_LEN (bis))
	af_xdp_buffer_free_no_next_batch (vm, bis, &n_free);
    }
  af_xdp_buffer_free_no_next_batch (vm, bis, &n_free);
}

static void
af_xdp_free_device (af_xdp_device_t *ad)
{
...
af_xdp_delete_if (vlib_main_t *vm, af_xdp_device_t *ad)
{
  ...
  if (af_xdp_remove_program (ad) != 0)
    {
      af_xdp_log (VLIB_LOG_LEVEL_ERR, ad,
		  "Error while removing XDP program, keeping interface for retry");
      return VNET_API_ERROR_SYSCALL_ERROR_6;
    }
  ...
  af_xdp_device_input_reset (ad);

  /* Reclaim VPP buffers from fill queues after the kernel path is detached. */
  for (i = 0; i < ad->rxq_num; i++)
    af_xdp_rxq_reclaim_fill_queue (vm, vec_elt_at_index (ad->rxqs, i));
  ...
  af_xdp_free_device (ad);
  return 0;
}

static int
af_xdp_update_tx_max_segs (af_xdp_create_if_args_t *args, af_xdp_device_t *ad)
{
#ifdef bpf_xdp_query_opts__last_field
  struct bpf_xdp_query_opts opts = { .sz = sizeof (opts) };
  int err = bpf_xdp_query (ad->linux_ifindex, 0, &opts);

  if (err)
    {
      args->rv = VNET_API_ERROR_SYSCALL_ERROR_4;
      args->error = clib_error_return_unix (0, "bpf_xdp_query(%s) failed", ad->linux_ifname);
      return -1;
    }

  if (opts.xdp_zc_max_segs == 0)
    {
      args->rv = VNET_API_ERROR_SYSCALL_ERROR_4;
      args->error = clib_error_return (
	0, "multi-buffer zero-copy cannot enforce device TX segment limit on %s", ad->linux_ifname);
      return -1;
    }

  ad->tx_max_segs = opts.xdp_zc_max_segs;
  return 0;
#else
  args->rv = VNET_API_ERROR_SYSCALL_ERROR_4;
  args->error = clib_error_return (
    0, "multi-buffer zero-copy requires libbpf xdp_zc_max_segs support on %s", ad->linux_ifname);
  return -1;
#endif
}
		     ...
		     bool enable_frags)
{
  ...
  vlib_log (VLIB_LOG_LEVEL_DEBUG, af_xdp_main.log_class,
  ...
  vlib_log (VLIB_LOG_LEVEL_DEBUG, af_xdp_main.log_class, "%s: Loaded XDP program '%s'",
  ...
  if (enable_frags)
	...
	{
	  args->rv = VNET_API_ERROR_SYSCALL_ERROR_5;
	  args->error = clib_error_return (0, "xdp_program__set_xdp_frags_support(%s) failed: %s",
  ...
  /* Attach the program. XDP_MODE_UNSPEC prefers native XDP and falls back to
   * generic; skb-mode forces the generic path (e.g. buggy native drivers). */
  enum xdp_attach_mode attach_mode =
    (args->flags & AF_XDP_CREATE_FLAGS_SKB_MODE) ? XDP_MODE_SKB : XDP_MODE_UNSPEC;
  vlib_log (VLIB_LOG_LEVEL_DEBUG, af_xdp_main.log_class,
	    "%s: Attaching XDP program to interface (ifindex=%d, mode=%d)", ad->linux_ifname,
	    ad->linux_ifindex, attach_mode);
  err = xdp_program__attach (xdp_prog, ad->linux_ifindex, attach_mode, 0);
  if (err)
    {
...
af_xdp_create_queue (vlib_main_t *vm, af_xdp_create_if_args_t *args, af_xdp_device_t *ad, int qid)
{
  ...
  /* Force generic/SKB-mode XDP for the libxsk default-program attach path;
   * the libxdp custom-program path handles this in af_xdp_load_program(). */
  ...
  if (opt.flags & XDP_OPTIONS_ZEROCOPY)
    {
      ...
      if ((ad->flags & AF_XDP_DEVICE_F_MULTI_BUFFER) && af_xdp_update_tx_max_segs (args, ad))
	goto err2;
    }

  if (ad->flags & AF_XDP_DEVICE_F_BUSY_POLL)
    {
      clib_error_t *err = af_xdp_set_busy_poll (ad, fd, qid);

      if (err)
	{
	  af_xdp_log (VLIB_LOG_LEVEL_WARNING, ad, "busy polling disabled: %U", format_clib_error,
		      err);
	  clib_error_free (err);
	  ad->flags &= ~AF_XDP_DEVICE_F_BUSY_POLL;
	  ad->busy_poll_usecs = 0;
	  ad->busy_poll_budget = 0;
	}
    }
...
af_xdp_create_if (vlib_main_t *vm, af_xdp_create_if_args_t *args)
{
  ...
  if ((args->flags & AF_XDP_CREATE_FLAGS_MAC_REUSE) &&
      ((ret = af_xdp_get_mac_address (args->linux_ifname, reuse_mac)) != 0))
    {
      args->rv = ret;
  ...
  if (prog)
    {
      ...
      vlib_log (VLIB_LOG_LEVEL_DEBUG, af_xdp_main.log_class,
		...
		enable_frags);
      ret = af_xdp_remove_program (ad);
      if (ret)
	{
	  args->rv = VNET_API_ERROR_SYSCALL_ERROR_6;
	  args->error = clib_error_return (0, "failed to remove XDP program from %s: ret %d",
					   ad->linux_ifname, ret);
	  af_xdp_free_device (ad);
	  goto err1;
	}
      if (af_xdp_load_program (args, ad, prog, enable_frags))
	{
	  af_xdp_free_device (ad);
	  goto err1;
	}
    ...
    {
      vlib_log (VLIB_LOG_LEVEL_DEBUG, af_xdp_main.log_class, "%s: No XDP program to load",
		ad->linux_ifname);
    }
  ...
  ad->tx_max_segs = AF_XDP_TX_COPY_MAX_SEGS;
  if (!args->name)
    {
...
err2:
  ret = af_xdp_delete_if (vm, ad);
  if (ret)
    {
      args->rv = ret;
      args->error = clib_error_return (
	args->error, "failed to roll back AF_XDP interface creation: ret %d", ret);
    }
err1:
...
af_xdp_interface_admin_up_down (vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
  ...
  uword was_up = (ad->flags & AF_XDP_DEVICE_F_ADMIN_UP) != 0;
